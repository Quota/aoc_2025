...
(comment
  (hier-weiter "`parse-area` fertig dokumentieren")
...
(defn get-url
  [day]
  (str "https://adventofcode.com/2025/day/" day "/input"))
...
; from: https://clojure.atlassian.net/browse/CLJ-2658
(defn distinct-by
  "Returns a lazy sequence of the elements of coll with duplicates,
  determined by (f item), removed. If (f item) produces the same
  result for multiple items, only the first item will be retained.
   ...
   (fn [rf]
     (let [seen (volatile! #{})]
       (fn
         ...
         ([result input]
          (let [finput (f input)]
            (if (contains? @seen finput)
              result
              (do (vswap! seen conj finput)
                  (rf result input)))))))))
  ([f coll]
   (let [step (fn step [xs seen]
                (lazy-seq
                  ((fn [[head :as xs] seen]
                     (when-let [s (seq xs)]
                       (let [fhead (f head)]
                         (if (contains? seen fhead)
                           (recur (rest s) seen)
                           (cons head (step (rest s) (conj seen fhead)))))))
                   xs seen)))]
     (step coll #{}))))
